...
//*                   Версия 2.1 от 10.12.2017  (Без щелчков)        *
...
while (i < m_len)
  ...
  if (n1 > 0) //Нота, иначе - пауза (и ничего не генерировать)
     ...
     OCR1A = TCNT1 + n1; //Записать новую константу в регистр сравнения A таймера 1
  ...
  PORTB &= ~(1 << PB3); //Нулевой уровень на выходе PB3 - может быть слышен щелчок
}
...
if (pause == 0) //Идёт процесс формирования ноты (не пауза)
   ...
   if (strobe) //Звук сейчас заблокирован стробом
	  ...
	  OCR1A = TCNT1 + n1;  //Инициализировать длительность нового полупериода для тона ноты
	  OCR1B = TCNT1 + T1B_1; //Занести в регистр сравнения B таймера 1 длительность отсутствия строба
	  ...
	  TCCR1B = 0; //Остановить таймер 1
	  OCR1B = TCNT1 + T1B_0; //Занести в регистр сравнения B таймера 1 длительность строба
	  TCCR1B |= (1 << CS11);    //Запустить таймер 1
      TIMSK  &= ~(1 << OCIE1A); //Запретить прерывания по совпадению A таймера 1
	  } 
   }  
